...
export async function startServer(
	...
	const env: NodeJS.ProcessEnv = {
		...
		TEST_MODE: '1',
		// admin-key.ts refuses to boot without ADMIN_KEY (≥16 chars).
		// Set the test value the harness uses for X-Admin-Key.
 ...
 * boot (admin-key.ts deliberately fails closed; the old fallback was
 * the vulnerability that module fixed).
 */
