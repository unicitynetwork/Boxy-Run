/**
 * Challenge lobby page — two clean sections:
 *
 * 1. **Play a Bot**: List of always-available bots. Click to play instantly.
 * 2. **Play a Friend**: Generate a shareable link. No need to be on the
 *    same page at the same time.
 *
 * Incoming challenges appear via REST polling (2s). WS is courtesy only.
 */
...
declare global {
	interface Window {
		...
		challengeBot: (name: string) => void;
		acceptChallenge: (id: string, btn: HTMLElement) => void;
		declineChallenge: (id: string, btn: HTMLElement) => void;
		createLink: () => void;
	}
}
...
let bots: { name: string; busy: boolean; skill: string }[] = [];
let humans: string[] = [];
...
// Inline "Connect Wallet" button on this page so users who land here
// directly don't have to detour to index.html.
document.getElementById('challenge-connect-btn')?.addEventListener('click', () => {
...
function handleMsg(msg: ServerMessage): void {
	switch (msg.type) {
		case 'challenge-received':
			pollIncomingChallenges(); // trigger immediate poll
			break;
...
function renderBots(): void {
	...
	const skillColor: Record<string, string> = { god: '#e879f9', expert: '#ff5e5e', medium: '#ff9534', beginner: '#4ade80' };
...
async function challengeBot(name: string): Promise<void> {
	...
	try {
		...
		// Bot should auto-accept within ~2s. WS challenge-start will redirect.
		// Fallback timeout in case WS is dead.
		setTimeout(() => {
			if (pendingBot) {
				clearBotPending();
...
function clearBotPending(): void {
	pendingBot = null;
	renderBots();
}

// ─── Incoming challenges (REST-polled) ──────────────────────────────

let lastIncomingIds = '';

async function pollIncomingChallenges(): Promise<void> {
	if (!myNametag) return;
	try {
...
function renderIncomingChallenges(challenges: any[]): void {
	const box = $('incoming-challenges');
	const ids = challenges.map((c: any) => c.id).join(',');
	...
	box.innerHTML = challenges.map((ch: any) => {
		...
		const label = isLink
			? `<strong style="color:var(--cyan)">${esc(ch.from)}</strong> has an open challenge${wagerText}${boText}`
			: `<strong style="color:var(--cyan)">${esc(ch.from)}</strong> challenges you${wagerText}${boText}`;
...
async function acceptChallenge(id: string, btn: HTMLElement): Promise<void> {
...
async function declineChallenge(id: string, btn: HTMLElement): Promise<void> {
	btn.closest('.challenge-incoming')?.remove();
	try {
...
async function showJoinUI(code: string): Promise<void> {
	...
	try {
		...
		$('join-body').innerHTML = `
			<div style="text-align:center">
				...
				${myNametag
					? `<button class="btn btn-primary" id="join-accept" style="padding:14px 32px;font-size:13px">Accept Challenge</button>`
					: `<div style="color:var(--text3);font-size:13px">Connect your wallet on the <a href="index.html" style="color:var(--cyan)">home page</a> first.</div>`
				}
			</div>`;
		document.getElementById('join-accept')?.addEventListener('click', () => {
			const btn = document.getElementById('join-accept') as HTMLButtonElement;
...
async function createLink(): Promise<void> {
	...
	try {
		...
		document.getElementById('link-url')!.addEventListener('click', function() { (this as HTMLInputElement).select(); });
		...
		linkPollTimer = setInterval(async () => {
			...
			try {
				const r = await fetch(`/api/challenge-links/${data.code}`);
				if (!r.ok) return;
				const info = await r.json();
				if (info.accepted) {
					clearLinkUI();
					// WS challenge-start should redirect. Show backup message.
